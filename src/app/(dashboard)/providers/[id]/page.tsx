...
import { ServerCog, Package, KeyRound, Activity, AlertTriangle, ChevronLeft, CheckCircle2, XCircle } from "lucide-react";
...
export default function ProviderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: stats,    loading: statsLoading } = useProviderStats(id);
          ...
          {stats?.lastError && (
            <div className="mt-3 card card-p flex items-start gap-2 border-red-500/20 bg-red-500/5">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">{stats.lastError.error}</p>
              </div>
            </div>
          )}
        </div>
              ...
              <ul className="divide-y divide-border">
                {provider.products?.map((pp) => (
                  <li key={pp.id} className="flex items-center justify-between px-4 py-2.5 gap-2">
                    <div>
                      <p className="text-xs text-foreground">{pp.product?.name ?? pp.productId}</p>
                      <code className="text-[10px] text-muted-foreground">{pp.externalRef}</code>
                    </div>
                    <div className="flex items-center gap-1.5">
                      {pp.active ? <CheckCircle2 className="w-3.5 h-3.5 text-green-400" /> : <XCircle className="w-3.5 h-3.5 text-muted-foreground" />}
                    </div>
                  </li>
                ))}
              </ul>
